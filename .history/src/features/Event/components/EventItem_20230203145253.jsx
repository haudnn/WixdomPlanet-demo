...
const EventItem = ({name, reward, file, introduction, during}) => {
    ...
    useEffect(() => {
        const startTimeFormat = formatDate(during.startTime);
        const endTimeFormat = formatDate(during.endTime);
        const currentTimeFormat = formatDate(new Date());
        let date1 = new Date(endTimeFormat).getTime();
        let date2 = new Date(currentTimeFormat).getTime();
        if (date1 < date2) {
        ...
        <div className="col l-4">
            <Link to="/" className="block rounded-xl p-[2px] cursor-pointer bg-[#2c2c2e] relative h-full">
                <div className="rounded-xl p-3 bg-[#2c2c2e] h-full flex flex-col">
                   <div>
                        <img src={file} alt="img" className="w-full h-full object-cover rounded-lg"/>
                   </div>
                   <div className="flex items-center box-border">
                        <span style={{display:"-webkit-box", "-webkit-line-clamp" : "2", "-webkit-box-orient" :"vertical"}} className="break-words text-white text-xl mt-3 mb-4 font-medium flex-1 max-w-full overflow-hidden text-ellipsis ">{name}</span>
                   ...
                   <div className="flex mt-[26px]">
                        ...
                        <div className="flex flex-col flex-1 items-end">
                            ...
                            <span className="mt-1 text-[#dddde5] text-sm flex items-center">
                                <img src="https://wixdomplanet.io/images/ic_time.svg" alt="BUSD" className="w-4 h-4 mr-2"/>
                                <span>-</span>
                            </span>
                        </div>
                   </div>
                   <div className="mt-4 flex justify-end">
                        <div className="flex items-center">
                            <img src="https://static.wixdomplanet.io/public/images/Logo-wxp.png" alt="" className="rounded-[50%] w-6 h-6 max-w-full"/>
                            <span className="ml-2 text-[#fcfcfc] font-bold">WixdomPlanet</span>
                        </div>
                   </div>
                </div>
            </Link>
