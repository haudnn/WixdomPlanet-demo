        ...
        <ul className="flex items-center gap-x-10">
          {
            categorise.map(category => (
              <li onClick={() => setSelectedCategoryId(category.id)} key={category.id} name={category.name} className={`${selectedCategoryId === category.id ? "active" : ""} font-medium text-[#949499] px-3 py-[6px] cursor-pointer text-sm`}>
                {category.name}
              </li>
            ))
          }
